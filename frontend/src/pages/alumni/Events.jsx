...
const Events = () => {
    ...
    const handleEdit = (evt) => {
        setEditMode(true);
        setEditId(evt.id);
        // Format the ISO date string to fit the datetime-local input
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            let uploadedImagePath = formData.banner_image; // keep existing if no new upload
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            fetchData(); 
        } catch (err) { alert('Failed to delete event. Admin access required.'); }
    };
    ...
    return (
        <div style={{ maxWidth: '1000px', margin: '40px auto', padding: '20px', fontFamily: 'sans-serif', color: 'white' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
                ...
                <button onClick={handleAddNew} style={{ padding: '10px 20px', backgroundColor: '#28a745', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>+ Host Event</button>
            </div>
                    ...
                    <div key={evt.id} style={{ backgroundColor: '#1e1e2f', borderRadius: '8px', border: '1px solid #333', overflow: 'hidden', position: 'relative' }}>
                        ...
                        {role === 'admin' && (
                            <div style={{ position: 'absolute', top: '10px', right: '10px', display: 'flex', gap: '5px', zIndex: 10 }}>
                                <button onClick={() => handleEdit(evt)} style={{ background: '#ffc107', border: 'none', borderRadius: '3px', cursor: 'pointer', padding: '5px 10px', fontSize: '12px', fontWeight: 'bold', color: '#000' }}>Edit</button>
                        ...
                        <div style={{ padding: '20px' }}>
                            <h3 style={{ margin: '0 0 10px 0', fontSize: '20px', color: '#ffc107', paddingRight: role==='admin'?'100px':'0' }}>{evt.title}</h3>
                    ...
                    <div style={{ backgroundColor: '#1e1e2f', padding: '30px', borderRadius: '8px', width: '100%', maxWidth: '500px' }}>
                        ...
                        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginTop: '20px' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                                <label style={{ fontSize: '12px', color: '#aaa', marginBottom: '5px' }}>Event Banner Image (Leave empty to keep current)</label>
