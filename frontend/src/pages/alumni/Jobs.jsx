...
// 🔹 SMART MOCK DATA
const mockJobs = [
    { id: 'j1', title: 'Frontend Developer (React)', company: 'Zoho Corp', location: 'Chennai, TN', salary_range: '8 - 12 LPA', experience_required: '1-3 Years', description: 'Looking for a passionate React developer from SRM to join our core UI team. Experience with Tailwind is a huge plus.', apply_link: '#', postedBy: { name: 'Priya Menon' } },
    { id: 'j2', title: 'Data Scientist', company: 'Amazon', location: 'Hyderabad / Remote', salary_range: '15 - 22 LPA', experience_required: '2+ Years', description: 'Urgent requirement for Data Scientists. Must be proficient in Python, Pandas, and Machine Learning algorithms.', apply_link: '#', postedBy: { name: 'Rahul Sharma' } },
    { id: 'j3', title: 'Product Design Intern', company: 'Cred', location: 'Bangalore, KA', salary_range: '40k / month', experience_required: 'Fresher', description: 'Hiring fresh design talent. Need a strong portfolio showing UI/UX skills. PPOs will be offered based on performance.', apply_link: '#', postedBy: { name: 'Arjun Reddy' } },
];
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto font-sans animate-in fade-in duration-500">
                ...
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 mt-1 font-medium">
                            Explore roles shared by the global SRM Alumni network
                        </p>
                    </div>
                </div>
                <button 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {jobs.map((job) => (
                    <div key={job.id} className="group bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-2xl hover:-translate-y-1 hover:border-emerald-200 transition-all duration-300 relative flex flex-col">
                        
                        {/* Admin/Owner Controls */}
                        {(role === 'admin' || currentUserId === job.userId) && (
                            <div className="absolute top-6 right-6 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => handleEdit(job)} className="p-2 bg-amber-50 text-amber-600 rounded-xl hover:bg-amber-600 hover:text-white transition-colors">
                                    <Edit3 size={16} />
                                </button>
                                <button onClick={() => handleDelete(job.id)} className="p-2 bg-red-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-colors">
                                    <Trash2 size={16} />
                                </button>
                            </div>
                        )}
                        ...
                        <div className="mb-6 pr-12">
                            <h3 className="text-2xl font-bold text-gray-800 font-serif mb-2 group-hover:text-emerald-700 transition-colors leading-tight">{job.title}</h3>
                            <p className="text-emerald-600 font-black text-sm tracking-widest uppercase bg-emerald-50 inline-block px-3 py-1 rounded-lg">{job.company}</p>
                        </div>
                        ...
                        <p className="text-gray-500 text-sm line-clamp-3 mb-8 leading-relaxed flex-grow">
                            {job.description}
                        </p>
                        ...
                        <div className="mt-auto pt-6 border-t border-gray-50 flex justify-between items-center">
                            <div className="flex flex-col">
                                ...
                                <span className="font-bold text-gray-700 text-sm">{job.postedBy?.name || 'Alumni'}</span>
                            </div>
                            <a href={job.apply_link || "#"} target="_blank" rel="noreferrer" className="bg-emerald-700 text-white px-5 py-2.5 rounded-xl font-bold text-xs hover:bg-gray-900 transition-colors uppercase tracking-widest shadow-md">
                                Apply Now
                            </a>
                        </div>
                    </div>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-emerald-900/40 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
                    ...
                    >
                        ...
                        <div className="sticky top-0 bg-emerald-800 p-6 border-b border-emerald-900 flex justify-between items-center z-10">
                            <h2 className="text-2xl font-bold text-white font-serif">
                                {editMode ? 'Update Opportunity' : 'Post New Opening'}
                            </h2>
                            <button onClick={() => setShowModal(false)} className="p-2 hover:bg-emerald-700 rounded-full transition-colors text-emerald-100">
                                <X size={24} />
                            </button>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="md:col-span-2">
                                <InputField label="Job Title" name="title" value={formData.title} onChange={handleChange} required placeholder="e.g. Full Stack Developer" />
                            </div>
                            ...
                            <div className="md:col-span-2">
                                <InputField label="Application Link / Email" name="apply_link" value={formData.apply_link} onChange={handleChange} placeholder="URL or HR Email" />
                            </div>
                            ...
                            <div className="md:col-span-2 flex gap-4 pt-4">
                                ...
                                <button type="button" onClick={() => setShowModal(false)} className="flex-1 py-4 bg-gray-100 text-gray-600 rounded-2xl font-bold hover:bg-gray-200 transition-all font-sans">
                                    Cancel
                                </button>
                            </div>
                        </form>
...
const InputField = ({ label, name, value, onChange, placeholder, required = false }) => (
    <div className="flex flex-col gap-2">
        ...
        <input 
            ...
            required={required} 
            className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl focus:ring-2 focus:ring-emerald-200 focus:bg-white outline-none transition-all text-gray-700 font-sans" 
        />
    </div>
);
